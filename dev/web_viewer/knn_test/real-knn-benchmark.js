/**
 * Real KNN Package Benchmark - Testing actual hnswlib-wasm and closevector-web
 * Measures exact accuracy and CPU time against ground truth
 */
...
class RealKNNBenchmark {
    ...
    /**
     * Test HNSW-lib WASM implementation
     */
    async testHNSWLib() {
        console.log('\n🧪 Testing HNSWLib-WASM...');
        
        try {
            const HnswLib = require('hnswlib-wasm');
            
            const setupStartTime = performance.now();
            const setupStartMemory = process.memoryUsage().heapUsed / 1024 / 1024;
            
            // Initialize HNSW index
            const index = new HnswLib.HierarchicalNSW('l2', 512);
            ...
            for (const query of this.queryVectors) {
                ...
                index.setEf(200); // Set search parameter
    ...
    async testCloseVector() {
        ...
        try {
            const CloseVector = require('closevector-web');
            
            const setupStartTime = performance.now();
            const setupStartMemory = process.memoryUsage().heapUsed / 1024 / 1024;
            
            // Initialize CloseVector
            const cv = new CloseVector(512, 'cosine'); // dimensions, distance metric
            
            // Add vectors
            for (const vector of this.testVectors) {
                cv.add(vector.id, vector.vector);
            }
            ...
            for (const query of this.queryVectors) {
                ...
                const results = cv.search(query.vector, 8);
            ...
            const result = {
                implementation: 'CloseVector-Web',
                setup_time_ms: setupTime,
                setup_memory_mb: setupMemory,
                avg_search_time_ms: totalSearchTime / numQueries,
                total_search_time_ms: totalSearchTime,
     ...
     */
    async runBenchmark() {
        console.log('🚀 Real KNN Package Benchmark: Measuring Actual Performance\n');
    ...
    generateReport(results) {
        ...
        console.log('Implementation'.padEnd(20) + 'Precision@8'.padEnd(12) + 'NDCG@8'.padEnd(10) + 'Exact Hits'.padEnd(12) + 'Quality Rating');
        console.log('-'.repeat(80));
        ...
        console.log('Implementation'.padEnd(20) + 'Setup Time'.padEnd(12) + 'Avg Search'.padEnd(12) + 'Speedup'.padEnd(10) + 'Memory');
        console.log('-'.repeat(80));
        
        // Add exhaustive search baseline
        console.log(
            'Exhaustive Search'.padEnd(20) +
            '0.0ms'.padEnd(12) +
        ...
        console.log(`🏆 Best Accuracy: ${best.implementation} with ${(best.accuracy_metrics.precision_at_k * 100).toFixed(1)}% precision`);
        console.log(`⚡ Fastest Search: ${fastest.implementation} with ${fastest.avg_search_time_ms.toFixed(2)}ms average`);
        
        const fastestSpeedup = avgExhaustiveTime / fastest.avg_search_time_ms;
        console.log(`📈 Speed Improvement: ${fastestSpeedup.toFixed(1)}x faster than exhaustive search`);

        console.log('\n📝 INTERPRETATION:');
        console.log('• Precision@8: % of returned neighbors that are actually in the true top-8');
        console.log('• NDCG@8: Ranking quality score (1.0 = perfect, 0.5 = random)');
        console.log('• Exact Hits: Average number of exactly correct neighbors found');
        console.log('• Speedup: How much faster than brute-force exhaustive search');
        console.log('• This benchmark uses REAL packages with actual vector similarity algorithms');
        ...
        fs.writeFileSync('real_knn_benchmark_results.json', JSON.stringify(reportData, null, 2));
...
module.exports = { RealKNNBenchmark };
