/**
 * Real KNN Package Benchmark - Correct API Usage
 * Testing actual hnswlib-wasm and closevector-web packages with proper API calls
 */
...
class RealKNNBenchmark {
    ...
    async testHNSWLib() {
        ...
        try {
            ...
            const result = {
                implementation: 'HNSWLib-WASM (Real)',
    ...
    /**
     * Test CloseVector implementation using HNSWLib
     */
    async testCloseVector() {
        console.log('\n🧪 Testing CloseVector-Web (using built-in HNSWLib)...');
        ...
        try {
            ...
            console.log('   Initializing CloseVector HNSW index...');
            const cv = new CloseVectorHNSWWeb(512, 'l2'); // dimensions, space
            
            // Check if we need to load WASM
            if (typeof cv.loadHnswlibWasm === 'function') {
                await cv.loadHnswlibWasm();
            }
            
            // Initialize the index
            await cv.initIndex(4096, 16, 200, 123); // maxElements, M, efConstruction, randomSeed
            cv.setEfSearch(200);
            
            // Add vectors
            console.log('   Adding vectors to CloseVector index...');
            for (let i = 0; i < this.testVectors.length; i++) {
            ...
            for (const query of this.queryVectors) {
                ...
                const results = cv.searchKnn(query.vector, 8);
            ...
            const result = {
                implementation: 'CloseVector-Web (Real)',
                setup_time_ms: setupTime,
                setup_memory_mb: setupMemory,
                avg_search_time_ms: totalSearchTime / numQueries,
                total_search_time_ms: totalSearchTime,
     ...
     */
    async runBenchmark() {
        console.log('🚀 REAL KNN PACKAGE BENCHMARK: Exact Accuracy & CPU Time\n');
        console.log('Testing ACTUAL hnswlib-wasm and closevector-web packages');
    ...
    generateReport(results) {
        ...
        console.log(
            'Exhaustive Brute-Force'.padEnd(25) +
            '0.0ms'.padEnd(12) +
        ...
        if (results.length > 0) {
            ...
            console.log(`   - ${fastest.setup_memory_mb.toFixed(1)}MB memory overhead for index`);
        }

        console.log('\n📝 WHAT THESE NUMBERS MEAN:');
        console.log('• Precision@8: If you ask "find 8 most similar vectors", this % will actually be in the true top-8');
        console.log('• NDCG@8: Ranking quality - does it get the ORDER right? (1.0 = perfect, 0.5 = random)');
        console.log('• Exact Hits: On average, how many of the 8 returned are exactly what brute-force would find?');
        console.log('• Speedup: How much faster than checking every single vector (4096 comparisons per query)');
        console.log('• These are REAL measurements using actual npm packages, not mock data!');
        ...
        const reportData = {
            ...
            description: 'Real KNN benchmark using actual hnswlib-wasm and closevector-web packages',
            dataset: {
...
benchmark.runBenchmark()
    .then(results => {
        ...
        console.log('\n🎉 You now have EXACT accuracy and CPU time measurements for real KNN packages!');
        process.exit(0);
    })
    .catch(error => {
