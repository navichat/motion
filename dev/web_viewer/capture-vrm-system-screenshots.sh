...
# Script to capture VRM system screenshots using Playwright
# This demonstrates the working VRM avatar system without geometric fallbacks

echo "🚀 Starting VRM system screenshot capture..."
...
# Create a simple Node.js script for screenshot capture
cat > capture_vrm_screenshots.js << 'EOF'
...
echo "🖼️ Capturing VRM system screenshots..."
timeout 300s node capture_vrm_screenshots.js

if [ $? -eq 0 ]; then
    echo "✅ VRM screenshots captured successfully!"
    
    # List captured screenshots
    echo "📂 Generated screenshots:"
    ls -la test-results/*.png 2>/dev/null || echo "No PNG files found"
    
else
    echo "❌ VRM screenshot capture failed"
    exit 1
fi