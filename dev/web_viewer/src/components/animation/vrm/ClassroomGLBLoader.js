...
class ClassroomGLBLoader {
    ...
    async initialize(classroomPath = null) {
        console.log('🏗️ Loading classroom GLB model...');
    ...
    // Collision detection methods (similar to the procedural classroom)
    checkCollision(position, radius = 0.3) {
        if (!this.initialized) return null;
        
        // Check floor bounds first
        if (this.floorBounds && !this.isWithinFloorBounds(position)) {
            return {
                type: 'floor_boundary',
            ...
            if (mesh.userData.collision && mesh.userData.boundingBox) {
                if (characterBounds.intersectsBox(mesh.userData.boundingBox)) {
                    return {
                        ...
                        object: mesh,
                        meshType: mesh.userData.meshType,
    ...
    isWithinFloorBounds(position) {
        if (!this.floorBounds) return true; // No bounds means no restriction
    ...
    clampToFloorBounds(position) {
        if (!this.floorBounds) return position;
    ...
    showCollisionBounds() {
        console.log('📦 Showing collision bounds...');
        ...
        this.collisionMeshes.forEach((mesh, index) => {
            if (mesh.userData.boundingBox) {
                ...
                const material = new window.THREE.MeshBasicMaterial({
                    color: mesh.userData.meshType === 'wall' ? 0xff0000 : 
                           mesh.userData.meshType === 'furniture' ? 0x00ff00 : 0x0000ff,
                    wireframe: true,
                    transparent: true,
                    opacity: 0.5
                });
                
                const boundsMesh = new window.THREE.Mesh(geometry, material);
                boundsMesh.position.copy(center);
                boundsMesh.userData.isCollisionBounds = true;
                boundsMesh.name = `CollisionBounds_${index}`;
    ...
    hideCollisionBounds() {
        ...
        this.scene.traverse((child) => {
            if (child.userData.isCollisionBounds) {
                boundsToRemove.push(child);
            }
        });
        
        boundsToRemove.forEach(bounds => {
            this.scene.remove(bounds);
            if (bounds.geometry) bounds.geometry.dispose();
            if (bounds.material) bounds.material.dispose();
        });
        
        console.log(`🧹 Removed ${boundsToRemove.length} collision bound visualizations`);
    }
