// Enhanced FaceFormer Web Generator with Exact Python Weight Loading
// Integrates the weight loading system with the main generator
...
class FaceFormerWebGeneratorEnhanced {
    ...
    async initialize(options = {}) {
        ...
        try {
            ...
            if (useExactWeights) {
                const { FaceFormerWeightLoader, SimpleFaceFormerJS } = await this.loadWeightSystem();
    ...
    async loadWeightSystem() {
        // Dynamic import for weight loading system
        if (typeof window !== 'undefined') {
            // Browser environment - load as script
            return new Promise((resolve, reject) => {
                const script = document.createElement('script');
                script.src = './weight_loader.js';
                script.onload = () => {
                    resolve({
                        FaceFormerWeightLoader: window.FaceFormerWeightLoader,
                        SimpleFaceFormerJS: window.SimpleFaceFormerJS
                    });
                };
            ...
            // Node.js environment
            return require('./weight_loader.cjs');
        }
    }
    ...
    async generateWithExactWeights(audioFeatures, verticeEmb, oneHot, template) {
        console.log('🎯 Generating frame with exact Python weights...');
        ...
        return {
            ...
            consistency: 'perfect' // Since we're using exact weights
        };
    }
...
if (typeof window !== 'undefined') {
    window.FaceFormerWebGeneratorEnhanced = FaceFormerWebGeneratorEnhanced;
}

if (typeof module !== 'undefined' && module.exports) {
    module.exports = { FaceFormerWebGeneratorEnhanced };
}
...
async function createEnhancedFaceFormer(options = {}) {
    ...
    if (success) {
        ...
        console.log('📊 Status:', generator.getStatus());
        return generator;
    } else {
...
if (typeof window !== 'undefined') {
    window.createEnhancedFaceFormer = createEnhancedFaceFormer;
} else if (typeof module !== 'undefined' && module.exports) {
    module.exports.createEnhancedFaceFormer = createEnhancedFaceFormer;
}
