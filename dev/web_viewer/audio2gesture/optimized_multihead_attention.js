...
class OptimizedAudio2GestureAttention {
    ...
    async _computeTemporalAttention(queries, keys, values, numFrames) {
        const sequenceLength = queries[0].length;
        const featureDim = sequenceLength;
        
        // Stack all frame features into temporal matrices
        const stackedQueries = this._stackTemporal(queries, numFrames, featureDim);
        const stackedKeys = this._stackTemporal(keys, numFrames, featureDim);
        const stackedValues = this._stackTemporal(values, numFrames, featureDim);
        
        // Compute attention scores for temporal dependencies
        let temporalAttention;
        
        // Determine backend based on operation size
        const totalOps = numFrames * featureDim * featureDim;
        const useGPU = totalOps > this.operationThresholds[this.currentBackend];
        
        if (useGPU && (this.currentBackend === 'webgpu' || this.currentBackend === 'webnn')) {
            temporalAttention = await this._computeTemporalAttentionGPU(stackedQueries, stackedKeys, stackedValues, numFrames);
        } else {
            temporalAttention = await this._computeTemporalAttentionCPU(stackedQueries, stackedKeys, stackedValues, numFrames);
        }
        
        // Unstack results back to per-frame format
        return this._unstackTemporal(temporalAttention, numFrames, featureDim);
    }
    ...
    async _computeTemporalAttentionCPU(queries, keys, values, numFrames) {
        const featureDim = queries.length / numFrames;
        const result = new Float32Array(queries.length);
        ...
        for (let queryFrame = 0; queryFrame < numFrames; queryFrame++) {
            ...
            for (let d = 0; d < featureDim; d++) {
                ...
                for (let valueFrame = 0; valueFrame < numFrames; valueFrame++) {
                    const valueOffset = valueFrame * featureDim;
                    weightedSum += attentionWeights[valueFrame] * values[valueOffset + d];
                }
    ...
    async _computeTemporalAttentionGPU(queries, keys, values, numFrames) {
        ...
        try {
            ...
            const queryData = this._createTypedArray(queries);
            const keyData = this._createTypedArray(keys);
            const valueData = this._createTypedArray(values);
            ...
            const keyBuffer = this.device.createBuffer({
                size: this._getBufferSize(keys.length),
                usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
            });
            
            const valueBuffer = this.device.createBuffer({
                size: this._getBufferSize(values.length),
                usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
            });
            ...
            const dataType = this.supportsF16 ? 'f16' : 'f32';
            const shaderCode = `
                ${this.supportsF16 ? 'enable f16;' : ''}
                ...
                fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
                    ...
                    for (var keyFrame = 0u; keyFrame < numFrames; keyFrame++) {
                        ...
                        for (var d = 0u; d < featureDim; d++) {
                            score += ${dataType}(queries[queryOffset + d]) * ${dataType}(keys[keyOffset + d]);
                        }
                    ...
                    for (var d = 0u; d < featureDim; d++) {
                        ...
                        for (var valueFrame = 0u; valueFrame < numFrames; valueFrame++) {
                            ...
                            weightedSum += normalizedWeight * ${dataType}(values[valueOffset + d]);
                        }
                        result[queryOffset + d] = ${dataType}(weightedSum);
                    }
                }
            `;
            ...
            commandEncoder.copyBufferToBuffer(resultBuffer, 0, readBuffer, 0, this._getBufferSize(queries.length));
            this.device.queue.submit([commandEncoder.finish()]);
            
            await readBuffer.mapAsync(GPUMapMode.READ);
            const arrayBuffer = readBuffer.getMappedRange();
            
            const resultData = new (this.supportsF16 ? Float16Array : Float32Array)(queries.length);
            if (this.supportsF16) {
    ...
    async _computeAttentionWebGPU(query, key, value) {
        try {
            ...
            // Write data to buffers with validation
            const queryTyped = this._createTypedArray(queryFlat);
            const keyTyped = this._createTypedArray(keyFlat);
            const valueTyped = this._createTypedArray(valueFlat);
            
            this.device.queue.writeBuffer(queryBuffer, 0, queryTyped);
            this.device.queue.writeBuffer(keyBuffer, 0, keyTyped);
            this.device.queue.writeBuffer(valueBuffer, 0, valueTyped);
            ...
            this.device.queue.writeBuffer(paramsBuffer, 12, new Float32Array([this.scalingFactor]));
            
            console.log(`🎮 WebGPU shader params: seq_len=${sequenceLength}, num_heads=${this.config.numHeads}, head_dim=${this.headDim}, scaling=${this.scalingFactor}`);
            
            // Validate critical parameters that could cause overflow
            if (this.headDim > 2048) {
                console.warn(`⚠️ Very large head_dim (${this.headDim}) may cause indexing issues`);
            }
            const maxIndex = sequenceLength * this.config.numHeads * this.headDim;
            if (maxIndex > totalSize) {
                console.error(`❌ Index overflow: maxIndex=${maxIndex} > totalSize=${totalSize}`);
            }
        ...
        const workgroupsX = Math.ceil(this.config.numHeads / 16);
        const workgroupsY = Math.ceil(sequenceLength / 16);
        
        console.log(`🎮 WebGPU dispatch params: numHeads=${this.config.numHeads}, sequenceLength=${sequenceLength}, hiddenDim=${this.config.hiddenDim}`);
        console.log(`🎮 WebGPU workgroups: X=${workgroupsX}, Y=${workgroupsY}, totalSize=${totalSize}`);
        console.log(`🎮 WebGPU buffer size: ${this._getBufferSize(totalSize)} bytes`);
        ...
        commandEncoder.copyBufferToBuffer(outputBuffer, 0, readBuffer, 0, this._getBufferSize(totalSize));
        this.device.queue.submit([commandEncoder.finish()]);
        ...
        // Debug: Check GPU computation results
        let gpuNanCount = 0;
        const gpuSampleValues = [];
        for (let i = 0; i < resultData.length; i++) {
            if (!Number.isFinite(resultData[i])) {
                gpuNanCount++;
            }
            if (gpuSampleValues.length < 10) {
                gpuSampleValues.push(resultData[i]);
            }
        }
        console.log(`🎮 WebGPU computation result: ${gpuNanCount}/${resultData.length} non-finite (${(gpuNanCount/resultData.length*100).toFixed(1)}%)`);
        console.log(`📊 Sample GPU result values:`, gpuSampleValues);
        ...
        readBuffer.destroy();            // Validate output for excessive NaN/Inf values
            const resultArray = Array.from(resultData);
            let nanCount = 0;
            for (let i = 0; i < resultArray.length; i++) {
                if (!Number.isFinite(resultArray[i])) nanCount++;
            }
            
            console.log(`🔍 Post-GPU Array conversion: ${nanCount}/${resultArray.length} non-finite (${(nanCount/resultArray.length*100).toFixed(1)}%)`);
            
            if (nanCount > resultArray.length * 0.1) {
                console.warn(`⚠️ WebGPU: Output contains too many NaN/Inf values (${nanCount}/${resultArray.length}), using fallback`);
                return await this._computeAttentionCPU(query, key, value);
            }

        // Reshape result back to original structure
        const reshapedResult = this._reshapeFromFlat(resultArray, batchSize, sequenceLength, this.config.hiddenDim);
        
        // Debug: Check reshaped result
        let reshapedNanCount = 0;
        let reshapedTotalCount = 0;
        for (let b = 0; b < reshapedResult.length; b++) {
            for (let s = 0; s < reshapedResult[b].length; s++) {
                for (let d = 0; d < reshapedResult[b][s].length; d++) {
                    reshapedTotalCount++;
                    if (!Number.isFinite(reshapedResult[b][s][d])) {
                        reshapedNanCount++;
                    }
                }
            }
        }
        console.log(`🔍 Post-reshape validation: ${reshapedNanCount}/${reshapedTotalCount} non-finite (${(reshapedNanCount/reshapedTotalCount*100).toFixed(1)}%)`);
