...
# Ultimate mic-first E2E via Vite self-serve with auto port selection

start_port="${USE_VITE_PORT:-5197}"
max_tries=20
...
echo "[ultimate:mic-first:vite:auto] Using port: $USE_VITE_PORT"
...
echo "[ultimate:mic-first:vite:auto] Running Playwright mic-first E2E..."
EXTERNAL_WEBSERVER=1 \
NO_WEBSERVER=1 \
USE_VITE=1 \
USE_VITE_PORT="$USE_VITE_PORT" \
timeout 900s npx playwright test --project=web_viewer-root-e2e dev/web_viewer/e2e-ultimate-mic-first.spec.js --reporter=line
