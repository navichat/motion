...
# ASR markers Playwright test via Vite self-serve with auto port selection

start_port="${USE_VITE_PORT:-5199}"
max_tries=20
...
echo "[asr-markers:vite:auto] Using port: $USE_VITE_PORT"
...
echo "[asr-markers:vite:auto] Running Playwright ASR markers test..."
EXTERNAL_WEBSERVER=1 \
NO_WEBSERVER=1 \
USE_VITE=1 \
USE_VITE_PORT="$USE_VITE_PORT" \
timeout 900s npx playwright test --project=web_viewer-root-e2e dev/web_viewer/e2e-ultimate-markers-asr.spec.js --reporter=line
